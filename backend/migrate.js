...
db.exec("CREATE TABLE IF NOT EXISTS predictions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL REFERENCES users(id), match_id INTEGER NOT NULL REFERENCES matches(id), pred_home INTEGER NOT NULL, pred_away INTEGER NOT NULL, points_earned INTEGER DEFAULT 0, submitted_at DATETIME DEFAULT (datetime('now')), UNIQUE(user_id, match_id))");
db.exec("CREATE TABLE IF NOT EXISTS bonus (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL REFERENCES users(id) UNIQUE, winner_id TEXT DEFAULT NULL, top_scorer_id TEXT DEFAULT NULL, points_bonus INTEGER DEFAULT 0)");
